...
pow_ADq24 <- rbind(pow_ADq24, p_AD)
...
require(ggplot2)
pow_ADq24$MeanSp <- as.numeric(pow_ADq24$MeanSp)
pow_ADq24$power <- as.numeric(pow_ADq24$power)
pow_ADq24$typeI <- as.numeric(pow_ADq24$typeI)

# Add field with combination of dispersal & mortality 
pow_ADq24$DspMrt <- with(pow_ADq24,paste0(DspD," - ",MrtR))

# Add number of species in the metacommunity
pow_ADq24$spMeta <- ceiling(as.numeric(pow_ADq24$NumSp)*1.33)

g <- ggplot(pow_ADq24,aes(x=spMeta,y=power)) + geom_point(shape=19,aes(size=typeI,colour=DspMrt)) + facet_grid(Side ~ Type) +
...
g <- ggplot(pow_ADq24,aes(x=MeanSp,y=typeI)) + geom_point(shape=19,aes(size=power,colour=DspMrt)) + facet_grid(Side ~ Type)
g+theme_bw()


# Recalculate power from comp_ADq24
#
require(plyr)
hh <-function(x) {
  t <- nrow(x)
...
comp_ADq24$spMeta <- ceiling(as.numeric(comp_ADq24$NumSp)*1.33)
...
```{r power_GCq10test, eval=T,echo=F,message=T,warning=T}
setwd("Simul")
...
cc <- foreach(i=1:nrow(simul)) %dopar%
{
    powerNeutral_1T_GC(simul[i,],3,40.2,0.0,0.001,10)
}
