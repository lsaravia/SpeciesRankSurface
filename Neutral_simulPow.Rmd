...
```{r more_simul256_LogSerParms, eval=F,echo=F,message=F,warning=F}
setwd("Simul")

simul <- rbind(simul,simulNeutral_1Time(8,256,500,"L",20,F))
simul <- rbind(simul,simulNeutral_1Time(64,256,500,"L",20,F))
simul <- rbind(simul,simulNeutral_1Time(256,256,500,"L",20,F))

setwd(oldcd)
save.image()
```
Using the GC with q<=10

```{r power_GCq10test, eval=T,echo=F,message=T,warning=T}
setwd("Simul")
# sim1 <- data.frame(nsp=65,side=256,time=100,meta="L",spMeta=87,rep=3)
#
#cc <- powerNeutral_1T_GC(simul[1,],3,0.2,0.04,0.001,10)
