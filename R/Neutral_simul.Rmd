...
#
# Generates pomac.lin for exponential dispersal to compare hierarchical and neutral communities 
...
# Change base name
par[par$V1=="baseName",]$V2 <- "Exp452"
# Change model type
par[par$V1=="modType",]$V2 <- 4 # Hierarchical saturated 

# I have to run the model with the parameter ReplacementRate turned on!! take care
...
N=500*500*4 # assume a metacommunity with 1.000.000
a=260
lsf <- rand.neutral(N,a)
ff <- data.frame(sp="",FreqT=as.numeric(lsf),prob=0,Ord=1:length(lsf),Type="Etienne")
tot1 <- tot1[tot1$Type!="Etienne",]
tot1 <-rbind(tot1,ff)

require(ggplot2)
ggplot(data=tot1,aes(x=Ord,y=log(FreqT),col=Type)) + geom_line()

#lsf <- function(x) log(rad.logserf(tot1[tot1$Type=="BCI",]$Freq,a)(x))
#g + stat_function(fun=lsf)
#g

# Select only the raw data BCI+Cocoli+Sherman
#
